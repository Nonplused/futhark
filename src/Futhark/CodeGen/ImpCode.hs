...
data Code a = Skip
            ...
            | Copy VName (Count Bytes Exp) Space VName (Count Bytes Exp) Space (Count Bytes Exp)
              ...
              -- of bytes.
            | Write VName (Count Bytes Exp) PrimType Space Volatility Exp
            | SetScalar VName Exp
...
data ExpLeaf = ScalarVar VName
             | SizeOf PrimType
             | Index VName (Count Bytes Exp) PrimType Space Volatility
           deriving (Eq, Show)
...
index :: VName -> Count Bytes Exp -> PrimType -> Space -> Volatility -> Exp
