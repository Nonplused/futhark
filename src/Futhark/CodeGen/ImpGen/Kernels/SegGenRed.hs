...
prepareAtomicUpdateGlobal :: Maybe Locking -> [VName] -> Lambda ExplicitMemory
...
compileSegGenRed (Pattern _ pes) num_groups group_size space ops kbody = do
  ...
  h <- fmap (`Imp.var` int32) $
       dPrimV "h" $ Imp.unCount $ sum op_hs + localMemLockUsage group_size slugs
  ...
  sUnless (h .==. 0) $ do
    ...
    forM_ (zip3 slugs pes_per_op ops) $ \(slug, red_pes, op) -> do
      ...
      sIf (Imp.var num_histos int32 .==. 1) unitHistoCase $ do
        ...
        let lvl = SegThread num_groups group_size
            segred_space =
              SegSpace flat_gtid $
              segment_dims ++
        ...
        compileSegRed' (Pattern [] red_pes) lvl segred_space
